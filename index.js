...
const saludos = ['Hola', 'hola', 'Buenos', 'buenos', 'buenas', 'Buenas', '¿Cómo está?', '¿cómo está?', '¿Como esta?', '¿como esta?', 'como esta', 'Saludos', 'saludos', 'saluda', 'Saluda','¿Cómo estás?', '¿cómo estás?', '¿Como estas?', '¿como estas?', 'como estas'];
...
client.on('message', message => {
    if (numeros.some(n => message.body.includes(n))) {
        if(noRegistrados.some(r => message.body.includes(r))){
